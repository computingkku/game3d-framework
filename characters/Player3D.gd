...
@export var friction := 20.0          # ชะลอบนพื้น
...
func _physics_process(delta: float) -> void:
	...
	if input_vec != Vector2.ZERO:
		...
		action = "walk"
		if  input_vec.y>0:
			model.rotation_degrees.y = lerp(model.rotation_degrees.y,0.0,0.4)
		elif  input_vec.y<0 && model.rotation_degrees.y!=-180:
			model.rotation_degrees.y = lerp(model.rotation_degrees.y,180.0,0.4)	
		elif  input_vec.x<0:
			model.rotation_degrees.y = lerp(model.rotation_degrees.y,-90.0,0.4)
		elif  input_vec.x>0:
			model.rotation_degrees.y = lerp(model.rotation_degrees.y,90.0,0.4)
